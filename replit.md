...
- **My Visions System:** Outcome-first vision system where users create individual vision outcomes that evolve through a dynamic, AI-guided conversation across 5 categories guided by Coach Cal, a natural coaching AI persona. **Context Sufficiency Score (CSS):** Progress determined by context depth/richness (0-100%) rather than question count. CSS ≥70% in a category indicates sufficient depth. **5 Categories:** Each vision explores (1) Vision - what you want, (2) Emotion - how it feels, (3) Belief - why you deserve it, (4) Identity - who you must become, (5) Embodiment - somatic/sensory experience. Categories are invisible to user - no labels shown, only vague progress bar. **Coach Cal AI:** Generates personalized questions using natural, direct coaching language (10-15 words max, no spiritual jargon). Questions reference specific user details from previous responses. AI dynamically selects next category based on CSS gaps and coverage - can jump categories or revisit as needed. **Multi-Category Tagging:** Single response can address multiple categories simultaneously, crediting CSS across all relevant areas. **CSS Calculation:** Hybrid system with 5 weighted signals: specificity (30%), emotional depth (25%), sensory richness (20%), temporal markers (15%), action orientation (10%). AI analyzes responses for category coverage, backend computes scores. **Auto-Generated Content:** AI automatically generates vision title and categories after first flow (stays consistent), then regenerates summary and tagline after every subsequent flow. Flow = one session where user answers ≥1 question before exiting. **Progress Tracking:** Percentage-based (0-100%) overall completeness. Users can exit mid-flow and resume later. **Grounding Screen:** Contemplative grounding screen shown at start of every vision flow session. **Navigation Flow:** MyVisionsScreen (vision cards with % progress) → VisionDetail (title, categories, tagline, vision statement, Q&A history, 3-dot menu for edit/delete) → "Continue Flow" → Grounding → VisionFlow → VisionRecord (speech-to-text) → VisionEdit (review/edit with "Continue" or "Finish Later" options). **VisionDetail UI:** 3-dot meatball menu (top right) with "Edit Title" and "Delete Vision" options. Vision statement/summary displayed prominently in styled card. No progress bar on detail screen. **Database Schema:** `visions` table (title, categories array, context_depth JSONB, overall_completeness, vision_css, emotion_css, belief_css, identity_css, embodiment_css, all categories completed booleans, summary, tagline), `vision_responses` table (with category field and multi_category_tags array tracking which categories each response addresses).
