...
- **Dark Mode Theme:** Centralized theming system (`mobile/src/theme.ts`) with dark mode palette. Background: #0f0f0f (very dark), Surface: #1a1a1a (cards), Primary accent: #7c3aed (purple). All screens use centralized theme colors for consistency. NavigationContainer configured with custom DarkTheme for proper stack transitions and modal backgrounds.
